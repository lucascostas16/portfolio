# Financial & Data Analytics Portfolio

This repository showcases practical projects focused on **FP&A, financial reporting and data-driven decision making**.

The objective is to demonstrate analytical thinking, financial understanding and the ability to translate data into business insights.

---

## Areas of focus
- FP&A & Management Control
- Financial reporting & KPI analysis
- Data analytics applied to finance
- SQL & data modeling
- Automation with Python / Excel

---

## Featured projects

### 📊 FP&A Reporting Pack
**Description:**  
Design of a financial reporting pack including P&L, budget vs actual analysis, KPIs and variance explanations.

**Tools:** Power BI · Excel · SQL  
**Status:** Initial version under development  
➡️ Repository: `fpna-reporting`

---

### 📈 Financial & Risk Analysis
**Description:**  
Analysis of financial assets including returns, volatility, correlations and basic risk metrics to support investment decisions.

**Tools:** Python · Excel  
**Status:** Initial version under development   
➡️ Repository: `financial-risk-analysis`

---

### 🗄️ SQL Case Studies
**Description:**  
Practical SQL exercises focused on data extraction, transformation and analysis for business and financial use cases.

**Tools:** SQL Server  
**Status:** Initial version under development   
➡️ Repository: `sql-case-studies`

---

## Contact
- **LinkedIn:** https://www.linkedin.com/in/lucas-costas-gonz%C3%A1lez-51b259235/
- **Location:** Spain (Madrid time)
